(async () => {
  ...
  const storageKey = testStorageKey !== null
    ? `marine-radar-quiz-progress-test-${testStorageKey || "default"}-v1`
    : "marine-radar-quiz-progress-v1";
  const syncSettingsKey = testStorageKey !== null
    ? `marine-radar-sync-test-${testStorageKey || "default"}-v1`
    : "marine-radar-sync-v1";
  ...
  function renderControls() {
    ...
    els.bankMeta.textContent = `${choiceCount} 道选择题 · ${shortCount} 道大题 · ${bank.chapters.length} 个知识点`;
  ...
  function renderExamPanel() {
    ...
    if (!hasPaper) {
      els.examSummary.textContent = `按知识点占比抽取 ${examQuestionCount} 道选择题`;
      hide(els.examReport);
      return;
    }
  ...
  function renderExamReport(metrics = currentExamMetrics()) {
    ...
    chapterTitle.textContent = "知识点表现";
  ...
  function renderQuestion(question) {
    ...
    els.chapterBadge.textContent = `知识点 ${question.chapterIndex}`;
    ...
    if (question.figureRefs.length) {
      els.figureNotice.textContent = `原文提示参考附图 ${question.figureRefs.join("、")}。当前 PDF 转换未能导出附图。`;
